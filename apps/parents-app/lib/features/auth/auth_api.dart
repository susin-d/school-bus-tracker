...
class AuthApi {
  ...
  Future<VerifiedSession> signInWithCredentials({
    ...
    final role = _mapMobileRole(user['role']);
    if (role == null) {
      throw const FormatException('This account is not allowed in the mobile app.');
    }
    ...
    return VerifiedSession(
      token: token,
      userId: userId,
      fullName: user['fullName'] as String? ?? user['full_name'] as String? ?? 'Mobile User',
      role: role,
    );
  }
  ...
  Future<VerifiedSession> verifyOtp({
    ...
    return VerifiedSession(
      token: token,
      userId: userId,
      fullName: user['fullName'] as String? ?? 'Mobile User',
      role: role,
    );
  }
  ...
  Future<void> sendParentForgotPasswordOtp({
    required String email,
  }) async {
    await _client.post('/auth/forgot-password/parent-otp/send', {
      'email': email,
    });
  }

  Future<void> verifyParentForgotPasswordOtp({
    required String email,
    required String otp,
    required String newPassword,
  }) async {
    await _client.post('/auth/forgot-password/parent-otp/verify', {
      'email': email,
      'otp': otp,
  ...
  AppRole? _mapMobileRole(Object? role) {
    switch (role) {
      case 'parent':
