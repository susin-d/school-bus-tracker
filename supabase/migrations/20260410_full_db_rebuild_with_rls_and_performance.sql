-- Full DB migration bundle
-- Includes: base RLS policies, map/routing RLS, observability, planner runs,
-- parent password reset OTP table rebuild, and 20260409 performance optimizations.
...
alter table public.nightly_planner_runs
  ...
  add column if not exists processed_trips integer not null default 0,
  add column if not exists planned_trips integer not null default 0,
  add column if not exists skipped_trips integer not null default 0,
